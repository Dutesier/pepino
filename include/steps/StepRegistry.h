...
public:
    ...
    template <typename Callback> void registerStep(const std::string& patternStr, Callback callback)
    {
    ...
    // Execute a step: choose among all matching candidates the one with highest specificity.
    void executeStep(const std::string& stepText) const
    {
        ...
        if (candidates.empty())
        {
            throw std::runtime_error("No matching step found for: " + stepText);
        }
    ...
    int computeSpecificity(const std::string& patternStr) const
    {
        ...
        while (pos < patternStr.size())
        {
            ...
            else if (patternStr[pos] == '[')
            {
                ...
                {
                    // Weight is proportional to the size of the character class.
